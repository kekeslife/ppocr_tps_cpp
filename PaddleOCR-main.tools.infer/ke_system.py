...
class TextSystem(object):
    ...
    def __call__(self, img, cls=True, slice={}):
        ...
        # for i,b in enumerate(img_crop_list):
        #     cv2.imwrite("F:/crop_image"+str(i)+".jpg", img_crop_list[i])
        # logger.info(img_crop_list[0])
...
def sorted_boxes(dt_boxes):
    ...
    for i in range(num_boxes - 1):
        for j in range(i, -1, -1):
            # print("sorted:",_boxes[j + 1][0][1] , _boxes[j][0][1])
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        for index, img in enumerate(imgs):
            ...
            if is_visualize:
                ...
                minPoints = np.min(boxes, axis=1)
                
                for index, box in enumerate(boxes):
                    cv2.putText(img,txts[index],(int(minPoints[index][0]),int(minPoints[index][1])),cv2.FONT_HERSHEY_COMPLEX,2.0,(230,0,18),5)
