...
void Utility::VisualizeBboxes(const cv::Mat &srcimg,
  ...
  for (int n = 0; n < ocr_result.size(); n++) {
    ...
    for (int m = 0; m < ocr_result[n].box.size(); m++) {
      rook_points.push_back(cv::Point(int(ocr_result[n].box[m][0]), int(ocr_result[n].box[m][1])));
    }

    const cv::Point *ppt[1] = {&rook_points[0]};
    //std::cout << "ppt size " << (*ppt)[0].x << (*ppt)[0].y << " " << rook_points[0].x << rook_points[0].y << std::endl;
    int npt[] = { ocr_result[n].box.size() };
...
cv::Mat Utility::GetRotateCropImage(const cv::Mat &srcimage,
  ...
  //std::cout << "=== img_crop: " << std::endl;
  //std::cout << img_crop_width << "," << img_crop_height << std::endl;
...
std::vector<std::vector<int>> Utility::Mat2Vector(cv::Mat mat) {
    ...
    for (int i = 0; i < mat.rows; ++i) {
        ...
        for (int j = 0; j < mat.cols; ++j) {
            tmp.push_back(int(mat.at<float>(i, j)));
        }
...
void Utility::Print_vec_int(std::vector<std::vector<int>> box, const std::string& title) {
...
cv::Mat Utility::GetMinareaRectCrop(const cv::Mat& srcimage,std::vector<std::vector<int>> box) {
    std::vector<cv::Point> boxes;

    //Print_vec_int(box,"GetMinareaRectCrop");

    for (int i = 0; i < box.size(); i++) {
        boxes.push_back(cv::Point(box[i][0], box[i][1]));
    }

    auto bounding_box = cv::minAreaRect(boxes);
    ...
    //Print_vec_int(array, "GetMinareaRectCrop");
    
    std::sort(
        array.begin(), array.end(),
        [](std::vector<int> pos1, std::vector<int> pos2) { return (pos1[0] < pos2[0]); });
    //Print_vec_int(array, "GetMinareaRectCrop");
    int index_a = 0;
    int index_b = 1;
    int index_c = 2;
    ...
    std::vector<std::vector<int>> crop_box = { array[index_a],array[index_b],array[index_c],array[index_d] };
    /*crop_box.push_back({ array[index_a][0],array[index_a][1] });
    crop_box.push_back({ array[index_b][0],array[index_b][1] });
    crop_box.push_back({ array[index_c][0],array[index_c][1] });
    crop_box.push_back({ array[index_d][0],array[index_d][1]});*/
    //Print_vec_int(crop_box, "crop_box");
    return Utility::GetRotateCropImage(srcimage, crop_box);
}
...
void Utility::sorted_boxes(std::vector<OCRPredictResult> &ocr_result) {
    ...
    if (ocr_result.size() > 0) {
        for (int i = 0; i < ocr_result.size() - 1; i++) {
            for (int j = i; j >= 0; j--) {
                //std::cout << ocr_result[j + 1].box[0][1] << "," << ocr_result[j].box[0][1] << std::endl;
                if (abs(ocr_result[j + 1].box[0][1] - ocr_result[j].box[0][1]) < 10 &&
                    (ocr_result[j + 1].box[0][0] < ocr_result[j].box[0][0])) {
                    std::swap(ocr_result[j], ocr_result[j + 1]);
                    //std::cout << " swap " << std::endl;
                }
                else {
                    break;
    ...
    /*std::cout << "=== sorted_boxes" << std::endl;
    for (int i = 0; i < ocr_result.size(); i++) {
        for (int j = 0; j < ocr_result[i].box.size(); j++) {
            std::cout << i << ": " << ocr_result[i].box[j][0] << "," << ocr_result[i].box[j][1] << std::endl;
        }
    }*/
}
