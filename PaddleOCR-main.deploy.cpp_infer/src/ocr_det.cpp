...
void DBDetector::Run(cv::Mat &img,
  ...
  // Inference.
  //std::cout << "=== keke Inference: " << std::endl;
  ...
  //std::cout << "=== keke predictor s: " << std::endl;
  this->predictor_->Run();
  //std::cout << "=== keke predictor e: " << std::endl;
  ...
  //std::cout << "=== keke postprocess: " << std::endl;
  ...
  boxes = post_processor_.PolygonsFromBitmap(
      pred_map, bit_map, this->det_db_box_thresh_, this->det_db_unclip_ratio_,
      this->det_db_score_mode_,srcimg.cols,srcimg.rows);
  ...
  boxes = post_processor_.FilterTagDetResOnlyClip(boxes, ratio_h, ratio_w, srcimg);
  /*std::cout << ratio_h << "===" << ratio_w << std::endl;
  for (int i = 0; i < boxes.size(); i++) {
      std::cout << "size" << boxes[i].size() << std::endl;
